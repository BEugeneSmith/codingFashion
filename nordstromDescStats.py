...
class itemReduce(Props):
    ...
    # Extracts pattern if it exists then bins
        for pattern in self.patterns:
    ...
    # extracts texture if it is in name, then bins
        for texture in self.textures:
    ...
    # extracts fit if it exists, then bins
        for fit in self.fits:
...
class ordinate(Props):
    ...
    def __ordinate(self,val):
        ...
        if val == 'unsorted':
            return 0
        colors = ['Red','Orange','Yellow','Green','Blue','Purple','Black','Grey','White','Brown']
        arrays = [self.items,self.fits,self.textures,self.patterns,colors]
        for l in arrays:
            try:
                return(1 + l.index(val))
            except:
                next
    ...
    def __dfOrd(self):
        ...
        for i in self.df.columns.tolist():
            ...
            if i not in features:
                ...
                self.df[i] = map(lambda x: self.__ordinate(x),self.df[i].tolist())
